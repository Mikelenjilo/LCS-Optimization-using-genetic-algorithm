...
def generate_first_gen(population_size: int, motif: str) -> list[Chromosome]:
    ...
    for _ in range(population_size):
        ...
        seq.get_str_from_array()
        first_gen.append(seq)

    return first_gen


def evaluate_gen(min_chrom: str, first_gen: list[Chromosome], chromosomes: list[str]):
    for chrom in first_gen:
        l: int = 0
        for i in range(len(chrom.array)):
            if chrom.array[i] == 1:
                l += 1

        m: int = calculate_m(chrom.sequence, chromosomes)
        v: int = l + 30 * m
        K: int = len(chromosomes)
...
def calculate_m(sub_seq: str, chromosomes: list[str]) -> int:
    counter: int = 0
    for chrom in chromosomes:
        ...
        while i < len(sub_seq) and j < len(chrom):
            if sub_seq[i] == chrom[j]:
                i += 1
                j += 1
                k += 1
            else:
...
def generate_gens(mutation_rate: float, first_gen: list[Chromosome], selection_method: SelectionMethod,
    ...
    best_solution: Chromosome = first_gen[0]
    for a in first_gen:
        if a.fitness > best_solution.fitness:
            best_solution = a

    if selection_method == SelectionMethod.TOURNAMENT:
        current_gen: list[Chromosome] = list(first_gen)
        new_gen: list[Chromosome] = []
        for _ in range(max_gens):
            while len(new_gen) != len(current_gen):
                parent_1: Chromosome = tournament(current_gen)
                parent_2: Chromosome = tournament(current_gen)
...
def crossover(parent_1: Chromosome, parent_2: Chromosome) -> tuple[Chromosome, Chromosome]:
    ...
    child_1.get_str_from_array()
    child_2.get_str_from_array()
