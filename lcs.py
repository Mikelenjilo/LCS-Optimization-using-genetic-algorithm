from exact_method.helper_functions import get_small_seq, get_long_seq, get_seqs_array, get_seq_str, evaluate_seqs, evaluate_seq
...
class LCS:
    ...
    def genetic_algorithm(chromosomes: list[str], population_size: int, mutation_rate: float, max_gens: int,
        ...
        min_seq: str = get_small_seq(chromosomes)
        first_gen: list[Chromosome] = generate_first_gen(population_size, min_seq)
        evaluate_gen(min_seq, first_gen, chromosomes)
